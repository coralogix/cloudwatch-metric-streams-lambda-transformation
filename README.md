...
| `AWS_SECRET_NAME`                | ``      |                                                                                               | If an AWS secret name is provided, all the configuration variables except `LOG_LEVEL` will be read from this secret using the variable names as the secret keys.
...
| `AWS_ACCOUNTS_TO_LABELS`         | `true`  | `[["account_1",[["label_name_1", "label_value_1"], ["label_name_2", "label_value_2"]]], ...]` | If provided, these account-specific labels will be added to metrics (based on the account that published them) if a label is not already present on the metric. Provide tuples of tuples (each 2-element, and non-blank), each representing an account and an associated list of labels to add to resources for the account. For example. use to add a production environment label to all resources in an account known to only contain production resources.
